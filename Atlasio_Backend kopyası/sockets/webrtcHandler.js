...
module.exports = (io, logger = console) => {
    io.on('connection', (socket) => {
        logger.info(`[ATLASIO WEBRTC] Yeni bir bağlantı şifreli tünele girdi. Soket ID: ${socket.id}`);

        // Öğrenci ve Eğitmeni güvenli, izole bir derse (odaya) alma
        socket.on('join-lesson', ({ lessonId, userId }) => {
            socket.join(lessonId);
            logger.info(`[ATLASIO WEBRTC] Kullanıcı [${userId}], Ders [${lessonId}] odasına katıldı.`);
            ...
            socket.on('disconnect', () => {
                logger.warn(`[ATLASIO WEBRTC] Kullanıcı bağlantısı koptu. Soket ID: ${socket.id}`);
