...
function buildService(prismaOverrides = {}, jwtOverrides = {}, redisOverrides = {}) {
    const prisma = makePrisma(prismaOverrides);
    const jwt = { ...makeJwt(), ...jwtOverrides };
    const audit = makeAudit();
    const redis = { ...makeRedis(), ...redisOverrides };
    const notifications = makeNotifications();
    return { service: new auth_service_1.AuthService(prisma, jwt, audit, redis, notifications), prisma, jwt, audit, redis };
}
const mockUser = {
    id: 'user-1',
...
describe('AuthService.refresh', () => {
    it('geçerli refresh token ile yeni token çifti döner', async () => {
        ...
        const { service: svc, prisma: p, redis: r } = buildService({}, jwtService, { ...makeRedis(), get: jest.fn().mockResolvedValue('user-1') });
        p.user.findUnique.mockResolvedValue(mockUser);
        p.user.update.mockResolvedValue(mockUser);
