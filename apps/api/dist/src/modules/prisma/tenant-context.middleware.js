...
let TenantContextMiddleware = TenantContextMiddleware_1 = class TenantContextMiddleware {
    ...
    async use(req, _res, next) {
        ...
        try {
            await req.prisma.$executeRaw `SELECT set_config('app.current_tenant', ${tenantId}, true)`;
        }
        catch (err) {
            this.logger.warn(`RLS session config failed for tenant=${tenantId}: ${err.message}`);
        }
        next();
    }
};
