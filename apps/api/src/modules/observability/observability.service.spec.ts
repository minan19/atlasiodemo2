...
const makePrisma = () => ({
  ...
  $transaction: jest.fn(async (fns: any[]) => fns.map(() => 0)),
});
...
describe('ObservabilityService.generateComplianceReport', () => {
  it('uyumluluk raporu oluşturur', async () => {
    ...
    (prisma.$transaction as jest.Mock).mockResolvedValue([100, 30, 5, 2, 10]);
...
describe('ObservabilityService.getTenantDashboard', () => {
  it('tenant dashboard verisini döner', async () => {
    const { service, prisma } = buildService();
    (prisma.$transaction as jest.Mock).mockResolvedValue([10, 5, 20, 100, 8]);
