...
export default function LiveHubPage() {
  ...
  function handleJoinByCode() {
    ...
    if (!code) { setJoinError("Oturum kodu girin."); return; }
    if (code.length < 4) { setJoinError("Geçersiz kod."); return; }
    ...
    <main className="space-y-5">
      ...
      <header className="glass p-6 rounded-2xl border border-slate-200 hero">
        <div className="hero-content flex flex-wrap items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="pill pill-sm bg-emerald-50 border-emerald-200 text-emerald-700">
                {liveStudentCount} öğrenci aktif
              </span>
            </div>
            <h1 className="text-2xl font-bold">Canlı Dersler</h1>
            <p className="text-sm text-slate-500">
              WebRTC tabanlı gerçek zamanlı oturumlar · HD video · Entegre whiteboard
            </p>
            {/* Stat row */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
              {[
                { icon: "🔴", label: "Canlı Oturum", value: activeSessions.filter(s => s.status === "RUNNING").length.toString(), color: "text-rose-600", bg: "bg-gradient-to-br from-rose-50 to-rose-100/50 border-rose-200" },
                { icon: "⏰", label: "Planlandı", value: activeSessions.filter(s => s.status === "SCHEDULED").length.toString(), color: "text-amber-600", bg: "bg-gradient-to-br from-amber-50 to-amber-100/50 border-amber-200" },
                { icon: "👥", label: "Aktif Öğrenci", value: liveStudentCount.toString(), color: "text-emerald-600", bg: "bg-gradient-to-br from-emerald-50 to-emerald-100/50 border-emerald-200" },
                { icon: "✓", label: "Tamamlanan", value: endedSessions.length.toString(), color: "text-slate-600", bg: "bg-gradient-to-br from-slate-50 to-slate-100/50 border-slate-200" },
              ].map(stat => (
                <div key={stat.label} className={`rounded-xl border p-3 text-center shadow-sm ${stat.bg}`}>
          ...
          <div className="flex flex-wrap gap-2">
            <Link href="/whiteboard" className="btn-link text-sm">🧠 Akıllı Tahta</Link>
            {isInstructor && (
              <button
                className="btn-link text-sm bg-emerald-500/10 border-emerald-400 text-emerald-700"
                onClick={() => setShowCreateForm((v) => !v)}
              >
                {showCreateForm ? "✕ Vazgeç" : "➕ Yeni Oturum"}
              </button>
            )}
          </div>
        </div>
      ...
      {isInstructor && showCreateForm && (
        <div className="glass rounded-2xl border border-emerald-200 bg-emerald-50/60 p-4 space-y-3 animate-scale-in">
          <div className="text-sm font-semibold text-emerald-800">Yeni Canlı Oturum Başlat</div>
          <div className="flex gap-2">
            <input
              ...
              placeholder="Oturum başlığı (ör. Matematik — Ders 5)"
            ...
            >
              {creatingSession ? "Oluşturuluyor…" : "🚀 Başlat"}
            </button>
          </div>
          <p className="text-xs text-emerald-700">
            Oturum başlatıldıktan sonra öğrenciler bağlantı kodu veya link ile katılabilir.
          </p>
        </div>
      )}
      ...
      {!isInstructor && (
        <div className="glass rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-emerald-50/30 p-4">
          <div className="text-sm font-semibold mb-3">Oturuma Katıl</div>
          <div className="flex gap-2">
            <input
              ...
              placeholder="Oturum kodu gir (ör. MATH-01)"
              value={joinCode}
          ...
          <p className="text-xs text-slate-400 mt-2">
            Kodu eğitmeninizden veya ders davet bağlantısından alabilirsiniz.
          </p>
        </div>
      )}
        ...
        <section className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-bold text-slate-800 flex items-center gap-2">
              ...
              Aktif & Planlanan Oturumlar
              <span className="pill pill-sm">{activeSessions.length}</span>
            </h2>
            {!sessions && <span className="pill pill-sm bg-amber-50 border-amber-200 text-amber-700">Demo veri</span>}
          </div>
                  ...
                  <div key={s.id} className="glass rounded-2xl border border-slate-200 overflow-hidden hover:-translate-y-1 transition-all duration-300 group shadow-sm hover:shadow-md">
                    ...
                    <div className="p-4">
                      ...
                      <div className="text-xs text-slate-500 space-y-1.5">
                        ...
                        {s.participantCount !== undefined && s.participantCount > 0 && (
                          <div className="flex items-center gap-1.5">
                            👥 <span>{s.participantCount} katılımcı</span>
                      ...
                      <div className="mt-3 flex gap-2">
                        <Link
                          href={`/live/${s.id}`}
                          className={`btn-link text-xs flex-1 justify-center ${s.status === "RUNNING" ? "bg-gradient-to-r from-rose-500 to-orange-500 text-white border-rose-400 shadow-sm" : "bg-emerald-500/10 border-emerald-400 text-emerald-700"}`}
                        >
                          {s.status === "RUNNING" ? "🔴 Derse Gir" : "📅 Görüntüle"}
                        </Link>
            ...
            {endedSessions.map((s) => (
              <div key={s.id} className="glass rounded-xl border border-slate-200 p-3 opacity-75">
                <div className="flex items-center justify-between">
                  ...
                  <span className="pill pill-sm text-[10px]">Bitti</span>
                </div>
                <div className="text-xs text-slate-400 mt-1">{sessionTime(s)}</div>
                {s.participantCount !== undefined && (
                  <div className="text-xs text-slate-400">👥 {s.participantCount} katılımcı</div>
                )}
                <div className="flex gap-1 mt-2">
                  <Link href={`/live/${s.id}`} className="btn-link text-[11px]">Kaydı İzle</Link>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="glass rounded-2xl border border-slate-200 p-5 space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          {[
            ...
            <div key={card.title} className="glass rounded-xl border border-slate-200 p-4 space-y-2 hover:border-emerald-300 transition-colors">
              <div className="flex items-center gap-2">
