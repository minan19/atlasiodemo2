...
function statusBadge(status: string) {
  if (status === "APPROVED")
    return <span className="pill text-xs bg-emerald-100 text-emerald-700 border border-emerald-300">Onaylı</span>;
  if (status === "REJECTED")
    return <span className="pill text-xs bg-rose-100 text-rose-700 border border-rose-300">Reddedildi</span>;
  return <span className="pill text-xs bg-amber-100 text-amber-700 border border-amber-300">Bekliyor</span>;
}
    ...
    <main className="space-y-6">
      ...
      <header className="glass p-6 rounded-2xl border border-slate-200">
        <div className="space-y-2">
          <span className="pill w-fit text-xs bg-violet-100 text-violet-700 border border-violet-300">LLM Taslak Onayı</span>
          <h1 className="text-3xl font-bold tracking-tight">AI Soru Kuyruğu</h1>
          <p className="text-sm text-slate-600">Yapay zeka ile üretilen soruları incele, onaylayın veya düzelt.</p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="rounded-xl border border-violet-200 bg-gradient-to-br from-violet-50 to-violet-100/50 p-4 text-center shadow-sm">
          ...
          <div className="text-xs text-slate-500 mt-1">Toplam Taslak</div>
        </div>
        ...
        <div className="rounded-xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-emerald-100/50 p-4 text-center shadow-sm">
          ...
          <div className="text-xs text-slate-500 mt-1">Onaylı</div>
        </div>
      </div>
      ...
      {data?.length === 0 && (
        <div className="glass rounded-2xl border border-slate-200 p-12 flex flex-col items-center gap-3 text-center">
          ...
          <h2 className="text-lg font-semibold text-slate-700">Taslak kuyruk boş</h2>
          <p className="text-sm text-slate-500">AI yeni sorular ürettiğinde burada görünür.</p>
        </div>
      )}
